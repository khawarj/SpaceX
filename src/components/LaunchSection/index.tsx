...
interface ILaunchSection {
  launch: any;
  onSelect: (mission: any, flag: boolean) => void,
  canAdd: boolean,
  selected?: boolean
}

const LaunchSection = ({ launch, onSelect, canAdd, selected= false }: ILaunchSection) => {
  ...
  const openRocketTimeline = (e: any) => {
    e.preventDefault();
    setModal(true);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 text-center">
          {launch.links?.wikipedia && (
            <a href={launch.links.wikipedia} rel="noopener noreferrer" target={"_blank"}>
        ...
        {!shipToggle && launch?.ships?.length > 0 && (
          <FaArrowDown
      ...
      <Modal open={modal} onModalClose={() => setModal(!modal)} header={<span className="">Rocket Details</span>}>
